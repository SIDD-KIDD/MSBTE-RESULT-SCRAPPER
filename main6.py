...
def download_result(start_enum, end_enum):
    for enum in range(start_enum, end_enum + 1):
        ...
        try:
            ...
            if "Result Data not found, Please check Enrollment/Seat No. fed by you" in alert_text:
                continue
            elif ("Error in Posted Data, Please Try again" in alert_text) or ("The Captcha Typed does not match with that shown on the screen" in alert_text):


start_enum=input("Enter starting enrollment number: ")
end_enum =input("Enter ending enrollment number: ")
