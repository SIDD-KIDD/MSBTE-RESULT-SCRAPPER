...
def download_result(start_enum, end_enum):
    ...
    for enum in range(start_enum, end_enum + 1):
        open_website()
        captcha_wrong = False  # Reset captcha flag for each iteration
        while True:
        ...
        if captcha_wrong:
            print("captcha wrong executed")
            fill_info((enum - 1))
        else:
            print("captcha wrong else executed")
            fill_info(enum)
